...
import { Plus, Trash2, UserPlus, Info, Save, Settings as SettingsIcon, CreditCard } from 'lucide-react';
...
export default function InvoiceForm({ formData, setFormData, onSave, isSaving }: InvoiceFormProps) {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 space-y-6">
      ...
      {/* Configuration Switches: Tax & Bank Details */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div className="flex items-center justify-between p-3.5 bg-slate-50/70 border border-slate-200/70 rounded-2xl">
          <div className="space-y-0.5 pr-3">
            <h3 className="text-xs font-bold text-slate-800">Apply Tax (GST/VAT)</h3>
            <p className="text-[10px] text-slate-400 font-medium">Calculate tax rates on items</p>
          </div>
        ...
        <div className="flex items-center justify-between p-3.5 bg-slate-50/70 border border-slate-200/70 rounded-2xl">
          <div className="space-y-0.5 pr-3">
            ...
            <p className="text-[10px] text-slate-400 font-medium">
              {formData.show_bank_details !== false ? 'Shown on PDF & Preview' : 'Hidden from PDF & Preview'}
            </p>
          </div>
          <label className="relative inline-flex items-center cursor-pointer flex-shrink-0">
            <input 
              type="checkbox" 
              checked={formData.show_bank_details !== false}
              onChange={(e) => setFormData((prev: any) => ({ ...prev, show_bank_details: e.target.checked }))}
              className="sr-only peer" 
            />
            <div className="w-10 h-5.5 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-4.5 after:w-4.5 after:transition-all peer-checked:bg-indigo-600"></div>
          </label>
      ...
      <div className="border-t border-slate-100 pt-5 space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          <div>
            ...
            <span className="text-[11px] text-slate-400 font-medium">
              Printed on the bottom-left of the PDF alongside bank account details.
            </span>
          </div>
          <Link
            href="/settings"
            target="_blank"
            className="text-[11px] font-bold text-indigo-600 hover:text-indigo-800 inline-flex items-center hover:underline cursor-pointer"
          >
        ...
        <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
          ...
          <button
            ...
            onClick={async () => {
              try {
                ...
                const terms = formData.document_type === 'invoice' 
                  ? (s.default_terms_invoice || '1. Please pay within 30 days of invoice issue date.\n2. Payment can be made via Bank Transfer or UPI details listed below.\n3. Late payments may attract standard commercial interest.')
                  : (s.default_terms_quotation || '1. Quotation is valid for 30 days from date of issue.\n2. 50% advance payment required upon order confirmation.\n3. Delivery and execution schedule starts upon receipt of advance.');
                setFormData((prev: any) => ({ ...prev, notes: terms }));
              } catch (e) {
                console.error(e);
              }
            }}
            className="px-2.5 py-1 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-[11px] font-bold rounded-lg border border-indigo-100/80 transition-colors cursor-pointer"
          >
            Load Saved Defaults
          </button>
          <button
            ...
            onClick={() => setFormData((prev: any) => ({
              ...prev,
              notes: '1. Payment due within 30 days of invoice date (Net 30).\n2. Remit payment to bank account details specified below.\n3. Include invoice number in payment reference.'
            }))}
          ...
          <button
            ...
            onClick={() => setFormData((prev: any) => ({
              ...prev,
              notes: '1. 50% advance payment required upon booking/order confirmation.\n2. Remaining 50% balance payable upon project completion/delivery.\n3. All taxes as applicable.'
            }))}
            className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 text-[11px] font-semibold rounded-lg border border-slate-200 transition-colors cursor-pointer"
          >
            50% Advance & Balance
          ...
          <button
            ...
            onClick={() => setFormData((prev: any) => ({
              ...prev,
              notes: '1. Quotation is valid for 30 days from issue date.\n2. Prices are subject to confirmation upon final site inspection.\n3. Standard warranty of 1 year applies on all deliverables.'
            }))}
            className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 text-[11px] font-semibold rounded-lg border border-slate-200 transition-colors cursor-pointer"
          >
            Quotation (30 Days)
          ...
          <button
            ...
            onClick={() => setFormData((prev: any) => ({
              ...prev,
              notes: '1. Rented equipment must be returned in original condition.\n2. Security deposit refunded upon safe return inspection.\n3. Daily rental rates apply for overdue periods.'
            }))}
            className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 text-[11px] font-semibold rounded-lg border border-slate-200 transition-colors cursor-pointer"
          >
            Rental Terms
          ...
          {formData.notes && (
            <button
              type="button"
              onClick={() => setFormData((prev: any) => ({ ...prev, notes: '' }))}
              className="px-2 py-1 text-slate-400 hover:text-rose-600 text-[11px] font-semibold hover:underline cursor-pointer ml-auto"
            >
              Clear
            </button>
          )}
        ...
        <textarea
          rows={4}
          placeholder="e.g. 1. Please pay within 30 days.&#10;2. Goods once delivered are not returnable.&#10;3. Bank details and UPI listed below."
          value={formData.notes || ''}
          onChange={(e) => setFormData((prev: any) => ({ ...prev, notes: e.target.value }))}
          className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-mono text-xs leading-relaxed"
        ></textarea>
      </div>
